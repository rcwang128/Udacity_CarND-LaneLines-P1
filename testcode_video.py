...
def draw_lines(img, lines, color=[255, 0, 0], thickness=7):
    ...
    for line in lines:
        # for every single Hough lines detected in a frame:
        for x1,y1,x2,y2 in line:
            ...
            if slope > 0: # right lane
                ...
                #print('raw=', np.mean(slope_pos))
                slope_right = np.mean(slope_right)
                intercept_right = np.mean(intercept_right)
                #print(slope_right, intercept_right)
                if(~np.isnan(slope_right) and ~np.isnan(intercept_right)):
                ...
                #print('raw=', slope_neg, intercept_y_neg)
        #print(slope_right, intercept_right)
        #print('raw2=', slope_pos)
        #if (len(slope_pos) > 2):
        #    slope_right = slope_pos[abs(slope_pos - np.mean(slope_pos)) < np.std(slope_pos)]
            #slope_right = slope_pos[np.mean(slope_pos)]
            #print('pro=', slope_right)
    ...
    slope_left = np.mean(slope_left)
    intercept_left = np.mean(intercept_left)
