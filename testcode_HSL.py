...
line_edges2 = cv2.addWeighted(color_edges, 0.8, line_img, 1, 0)
line_edges = cv2.addWeighted(image, 0.8, line_img, 1, 0)

plt.subplot(241);plt.title('Original');plt.imshow(image)
plt.subplot(242);plt.title('HSL');plt.imshow(image_hsl)
plt.subplot(243);plt.title('Gray');plt.imshow(image_gray)
plt.subplot(244);plt.title('Canny Edges');plt.imshow(edges)
plt.subplot(245);plt.title('Masked Edges');plt.imshow(masked_edges)
plt.subplot(246);plt.title('Color edges');plt.imshow(color_edges)
plt.subplot(247);plt.title('Extrapolated Lines');plt.imshow(line_edges2)
#fig = plt.figure()
#ax = plt.plot()
#fig.savefig('test_images_output/solidyellowecurve2.png')
plt.subplot(248)
plt.title('Dected Lines on original image');plt.imshow(line_edges)
#plt.imshow(image_blank)
plt.show()
